...
with open(dir_config.full_transit_graphs_dir / "gtfs_graph_undirected.pkl", "rb") as f:
    g = pickle.load(f)

g = g.as_directed(mode="mutual")

vertex_df = (
    g.get_vertex_dataframe().reset_index().rename(columns={"vertex ID": "vertex_id"})
)
edge_df = g.get_edge_dataframe().reset_index().rename(columns={"edge ID": "edge_id"})
# %%
...
daily_summary_df["num_delayed_trains"] = daily_summary_df["num_delayed_trains"].fillna(
    0
)
# %%
...
# %%
vertex_df.to_parquet(dir_config.analysis_data_dir / "daily_vertex_attributes.parquet")
